...
import os
import pandas as pd
from pandas.io.orc import get_handle
...
class ShopifyApp:
    ...
    async def delete_product(self, client: httpx.AsyncClient, product_id: str, semaphore: asyncio.Semaphore) -> Dict[str, str | None]:
        mutation = """
        mutation deleteProductById($input: ProductDeleteInput!) {
        ...
        for attempt in range(3):
            async with semaphore:
                try:
                    ...
                    if 'errors' in data:  # Rate limit hit
                        print(f"Rate limit hit for product id: {product_id}. Retrying...")
                        delay = 2 * (2 ** attempt)  # Exponential backoff
                        await asyncio.sleep(delay)
                        continue  # Retry the request
                    ...
                    print(f"Error deleting product id: {product_id}")
                    print(f"Error Response: {e.response.text}")
                    print(f"Status: {e.response.status_code} | StatusText: {e.response.reason_phrase}")
                except Exception as e:
                    print(f"Error Message: {str(e)}")
    ...
    async def delete_products(self, product_ids: List[str]) -> List[Dict[str, str | None]]:
        semaphore = asyncio.Semaphore(10)
        async with httpx.AsyncClient() as client:
            tasks = [self.delete_product(client, product_id, semaphore) for product_id in product_ids]
            results = await asyncio.gather(*tasks)
        return results
...
if __name__ == '__main__':
    ...
    asyncio.run(s.async_delete_products())
    ...
    # df = pd.read_csv('./data/need_to_drop.csv')
